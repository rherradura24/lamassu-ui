...
export const DeviceList = ({ devices, dmsList, dmsEnrollerRequestInProgress, deviceRequestInProgress, refreshData }) => {
    ...
    const devicesTableColumns = [
        ...
        { key: "dms", title: "DMS ID", align: "center", type: "string", size: 2 },
    ...
    console.log(dmsKeyList);
    const devicesListRender = device => {
        var dmsContent = device.dms_id
    ...
    return (
        deviceRequestInProgress.status === status.SUCCEEDED && dmsEnrollerRequestInProgress.status === status.SUCCEEDED ? (
            devices.length > 0
                ? (
            ...
            <Grid container style={{ height: "100%", padding: "30px" }} spacing={2}>
                <Grid item xs={3}>
