            ...
            <Grid container spacing={3}>
                {
                    devices.map(device=>(
                        <Grid item xs={3}>
                            <DeviceCard style={{cursor: "pointer"}} onClick={()=>alert()}  id={device.id} alias={device.alias} description={device.description} tags={device.tags} icon={device.icon} iconColor={device.icon_color} remainigDaysBeforeExpiration={device.remainig_days_before_expiration}/>
                        </Grid>
                    ))
                }
            </Grid>
