        ...
        <Grid container style={{height: "100%", background: theme.palette.background.lightContrast}}>
            {
                selectedDevice === undefined ? (
                            ...
                            <Grid container alignItems={"center"} justifyContent="space-between" sx={{marginBottom: "35px"}}>
                                ...
                                <Grid item xs="auto" container spacing={4}>
                                    <Grid item xs="auto" container alignItems={"center"} spacing={2}>
                                        ...
                                        <Grid item>
                                            <GrayButton size="small" variant="contained" disableFocusRipple disableRipple endIcon={anchorEl ? <KeyboardArrowUpIcon /> : <KeyboardArrowDownIcon />} onClick={handleClick}>Actions</GrayButton>
                                            <Menu
                            ...
                            <Grid container spacing={3}>
                                {
                                    devices.map(device=>(
                                        <Grid item xs={3}>
                                            <DeviceCard style={{cursor: "pointer"}} onClick={()=>setSelectedDevice(device.id)}  id={device.id} alias={device.alias} description={device.description} tags={device.tags} icon={device.icon} iconColor={device.icon_color} remainigDaysBeforeExpiration={device.remainig_days_before_expiration}/>
                                        </Grid>
                                    ))
                                }
                            </Grid>
                    ...
                    <Grid item xs={12} container>
                        <DeviceInspector deviceId={selectedDevice}/>
                    </Grid>
                )
            }
        </Grid>
