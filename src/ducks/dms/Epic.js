...
const createDmsViaCsrEpic = action$ => action$.pipe(
    ofType(actions.CREATE_DMS_VIA_CSR_REQUEST),
    mergeMap(({payload}) => makeRequestWithActions(dmsApiCalls.createDmsViaCsr(payload), actions.CREATE_DMS_VIA_CSR_REQUEST)),
);

const createDmsViaFormEpic = action$ => action$.pipe(
    ofType(actions.CREATE_DMS_VIA_FORM_REQUEST),
    mergeMap(({payload}) => makeRequestWithActions(dmsApiCalls.createDmsViaForm(payload), actions.CREATE_DMS_VIA_FORM_REQUEST)),
);

const notifyCreateDmsSuccess = action$ => action$.pipe(
    ofType(actions.CREATE_DMS_VIA_FORM_REQUEST_SUCCESS, actions.CREATE_DMS_VIA_CSR_REQUEST_SUCCESS),
...
const getCertForApprovedDms = action$ => action$.pipe(
    ofType(actions.GET_ALL_DMS_SUCCESS),
    mergeMap(response => {return response.payload.filter(dms=>dms.status == "APPROVED").map(dms => dms.id)}), 
    mergeMap(dmsId => makeRequestWithActions(dmsApiCalls.getDmsCert({id: dmsId}), actions.GET_DMS_CERT, {dmsId: dmsId}))
);

/// General ERROR Notify
const notifyError = (action$, state$) => action$.pipe(
    ofType(actions.GET_ALL_DMS_ERROR, actions.UPDATE_DMS_STATUS_ERROR, actions.CREATE_DMS_VIA_FORM_REQUEST_ERROR, actions.CREATE_DMS_VIA_CSR_REQUEST_ERROR, actions.GET_DMS_CERT_ERROR ),
